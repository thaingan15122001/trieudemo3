  ...
  return (
    <>
      <nav
        ...
        className={`sticky top-0 z-[100] flex w-full items-center justify-between px-5 py-6 transition-opacity duration-700 md:fixed md:left-0 md:px-16 ${visible ? 'opacity-100' : 'opacity-0'}`}
        aria-hidden={!visible}
      >
        <div
          className={`absolute inset-0 -z-10 border-b border-white/10 bg-[#34282D] backdrop-blur-xl transition-opacity duration-500 md:bg-[#34282D]/80 md:${insideHero ? 'opacity-0' : 'opacity-100'}`}
          aria-hidden="true"
        />
